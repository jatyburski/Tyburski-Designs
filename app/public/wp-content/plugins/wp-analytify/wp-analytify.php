...
if ( ! class_exists( 'WP_Analytify' ) ) {
	...
	class WP_Analytify extends Analytify_General {
       ...
       if ( ! $manual_ua_code ) {
			return;
			 ...
			 if ( get_option( 'pa_google_token' ) && WP_ANALYTIFY_FUNCTIONS::get_UA_code() ) {
				 return;
			 }
			...
			if ( in_array( 'administrator', $roles ) ) {
				...
				// Use gtag script unless filterd.
    ...
    private function output_gtag_code( $UA_CODE ) {
      ...
      $accept_incoming = ( 'on' === $this->settings->get_option( 'linker_cross_domain_tracking', 'wp-analytify-advanced' ) ) ? 'true' : 'false'; ?>
      ...
      <script>
        ...
        const configuration = {
          ...
          'accept_incoming': <?php echo $accept_incoming; ?>
        };
      ...
      * @return $ga_code
		...
		public function admin_scripts( $page ) {
			...
			// for main page
			if ( $page == 'index.php' || $page == 'toplevel_page_analytify-dashboard' || $page == 'analytify_page_analytify-woocommerce' || $page == 'analytify_page_edd-dashboard' || $page == 'analytify_page_analytify-campaigns' || $page == 'analytify_page_analytify-goals' || $page == 'analytify_page_analytify-forms-dashboard' || $page == 'analytify_page_analytify-dimensions'  || $page == 'analytify_page_analytify-authors' || $page == 'analytify_page_analytify-events' || in_array( $post_type, $this->settings->get_option( 'show_analytics_post_types_back_end','wp-analytify-admin', array() ) ) ) {
      ...
      if ( 'on' == $this->settings->get_option( 'depth_percentage', 'wp-analytify-advanced' ) ) {
        ...
        wp_localize_script( 'scrolldepth-js', 'analytifyScroll', array(
          'permalink' => $permalink
        ));
      }
